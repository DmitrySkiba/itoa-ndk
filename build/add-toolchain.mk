...
$(if $(strip $($(_toolchain).defined)),\
  $(call ndk_error,Toolchain $(_name) defined in $(_parent) is\
...
$(if $(strip $($(_toolchain).setup)),,\
  $(call ndk_error, Toolchain $(_name) lacks a setup.mk in $(_dir)))

NDK_ALL_TOOLCHAINS += $(_name)
NDK_ALL_ABIS       += $(_abis)

# Records the list of toolchains that support a given ABI
#
$(foreach _abi,$(_abis),\
    $(call add-abi-var,$(_abi),TOOLCHAINS,$(call get-abi-var,$(_abi),TOOLCHAINS) $(_name)) \
)
